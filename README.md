...
**症状:**
- 运行 `eth-camera-app -D` 或正常退出时，程序卡在"清理资源..."阶段
- 或者退出时出现内核错误：`Unable to handle kernel NULL pointer dereference at virtual address 0x88`

**原因:**
DMA 引擎有"飞行中"的传输，在 `munmap` 帧缓冲区时导致内核崩溃。旧版本清理逻辑不够完善：
1. 只是简单地写 0 到控制寄存器，没有等待 VDMA 真正停止
2. 没有执行软复位确保 DMA 引擎完全停止
3. munmap 前没有足够的等待时间让飞行中的 AXI 事务完成

**解决方法:**
- 最新版本已修复此问题，`vdma_cleanup()` 会：
  1. 清除 Run 位，等待 HALTED
  2. 执行软复位，确保 DMA 引擎完全停止
  3. 使用内存屏障 + 延时（约 160ms）等待所有 AXI 事务完成
  4. 按正确顺序 munmap 和关闭 fd
- 请重新编译程序并部署。
